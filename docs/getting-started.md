...
本文档面向 **ARM64 Ubuntu Desktop + Quectel EC801E-CN**，从连接模组到收发第一条短信、打通第一通电话，按顺序操作即可。
...
| 物品 | 说明 |
|------|------|
| ARM64 设备 | 如 Orange Pi、树莓派、VMware Fusion ARM 虚拟机 |
| Ubuntu 24.04 Desktop | GNOME 桌面环境 |
...
1. 确认系统为 **Ubuntu 24.04 Desktop（GNOME）ARM64**。
2. 完成首次用户创建并登录桌面。
3. 连接 Wi‑Fi 或有线网络（安装依赖时需要）。
...
3. 将 EC801E 插入 USB 口（建议 USB 3.0，供电更稳）。
4. 开机登录后，先确认系统能看到模组：
...
## EC801E（ECM / RNDIS 模式）说明

Quectel **EC801E-CN** 多数固件仅支持 USB 模式 **ECM** 或 **RNDIS**，**不支持 QMI**。HiPi 会自动：

- 通过 **ModemManager** 读取信号、运营商、SIM 状态
- 通过 **AT 串口**（`/dev/ttyUSB*`）收发短信、解锁 PIN、外呼/挂断
...
```bash
hipi status
# 期望: modem_present=true, sms_backend="at" 或 "mmat", at_port="/dev/ttyUSB..."
